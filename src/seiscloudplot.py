...
def plot_spatial(events,eventsclusters,clusters,conf,plotdir):
    ...
    # Map size
    if conf.radius_map_plot is not None:
        safe_radius = conf.radius_map_plot
    else:
...
def plot_triangle(events,eventsclusters,clusters,conf,plotdir):
    ...
    plt.plot([-1.2247,1.2247,0.,-1.2247],[-0.7070,-0.7070,1.4144,-0.7070],c='k',lw=1.)
...
def plot_axis(events,eventsclusters,clusters,conf,plotdir):
    ...
    f.suptitle('Pressure (P), tension (T) and null (B) axis for seismicity clusters', fontsize=14)
